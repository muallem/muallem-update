                                        ...
                                        <div class="card-body">
                                            {{-- <form class="chat-input px-3" wire:submit.prevent='store'> --}}
                                                <ul class="nav nav-tabs nav-tabs-new2 d-flex justify-content-start" role="tablist">
                                                    @foreach ($lesson->lesson_details as $index => $item)
                                                        <li class="nav-item my-1" role="presentation" >
                                                            <a style="font-size: 23px;" class="nav-link {{($index == 0) ? 'active' : ''}}" data-bs-toggle="tab" href="#{{str_replace(' ', '_', $item->title ."-". $item->id)}}" aria-selected="true" role="tab" >
                                                                {{$item->title}}
                                                            </a>
                                                        </li>
                                                    @endforeach
                                                ...
                                                <div class="tab-content mt-3">
                                                    @foreach ($lesson->lesson_details as $index => $item)
                                                        <div class="tab-pane {{($index == 0) ? 'show active' : ''}}" id="{{str_replace(' ', '_', $item->title ."-". $item->id)}}" role="tabpanel">
                                                            <h4>{{$item->title}}</h4>
